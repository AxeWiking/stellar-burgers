...
describe('проверям добавление ингредиентов в конструкторе', () => {
    ...
    it('ингредиент булки должен появиться в конструкторе по кнопке "Добавить"', () => {
        const bun = cy.get(`[data-testid="ingredient-id-${constant.cratorBun._id}"]`);
        const addBunButton = bun.contains('button', 'Добавить');
        addBunButton.click();
        const topItem = cy.get('[data-testid="constructor-element-top"]');
        topItem.contains(`${constant.cratorBun.name} (верх)`).should('exist');
        const bottomItem = cy.get('[data-testid="constructor-element-bottom"]');
        bottomItem.contains(`${constant.cratorBun.name} (низ)`).should('exist');
    });

    it('ингредиент котлеты должен появиться в конструкторе по кнопке "Добавить"', () => {
        const ingredient = cy.get(`[data-testid="ingredient-id-${constant.biococlet._id}"]`);
        const addIngredientButton = ingredient.contains('button', 'Добавить');
        addIngredientButton.click();
        const listItems = cy.get('[data-testid="constructor-elements-list"]');
        listItems.contains(constant.biococlet.name).should('exist');
    });
});
...
describe('оформляем заказ', () => {
    ...
    beforeEach(() => {
        ...
        window.localStorage.setItem(
          'refreshToken', JSON.stringify('qwerty')
        );
        cy.setCookie('accessToken', 'secret');
        cy.intercept('POST', 'api/orders', {
           success: true,
           name: 'Шипящяя плесень',
  ...
  it('покупка бургера', function () {
    ...
    cy.get('[data-testid="constructor-element-top"]').
        contains(`${constant.cratorBun.name} (верх)`).should('exist');
    cy.get('[data-testid="constructor-element-bottom"]').
        contains(`${constant.cratorBun.name} (низ)`).should('exist');

    const coclet = cy.get(`[data-testid="ingredient-id-${constant.biococlet._id}"]`);
    const sauce = cy.get(`[data-testid="ingredient-id-${constant.spicySauce._id}"]`);    
    ...
    cy.get('[data-testid="constructor-elements-list"]').
        contains(constant.spicySauce.name).should('exist');
    cy.get('[data-testid="constructor-elements-list"]').
        contains(constant.biococlet.name).should('exist');
    ...
    cy.get('[data-testid="constructor-element-top-empty"]').
        contains(`${constant.cratorBun.name} (верх)`).should('not.exist');
    cy.get('[data-testid="constructor-element-bottom"]').should('not.exist');
    cy.get('[data-testid="constructor-element-bottom-empty"]').
        contains(`${constant.cratorBun.name} (низ)`).should('not.exist');
    cy.get('[data-testid="constructor-elements-list"]').
        contains(constant.spicySauce.name).should('not.exist');
    cy.get('[data-testid="constructor-elements-list"]').
        contains(constant.biococlet.name).should('not.exist');
  });
});
